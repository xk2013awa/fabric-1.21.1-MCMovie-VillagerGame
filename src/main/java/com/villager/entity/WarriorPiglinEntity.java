...
public class WarriorPiglinEntity extends HostileEntity implements GeoEntity{

    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    public WarriorPiglinEntity(EntityType<? extends HostileEntity> entityType, World world) {
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        ...
        controllers.add(
                new AnimationController<>(this, "attack_controller", state -> PlayState.STOP)
                        .triggerableAnim("attack1", RawAnimation.begin().then("animation.villager.warrior_piglin.melee_attack1", Animation.LoopType.PLAY_ONCE))
                        .triggerableAnim("attack2", RawAnimation.begin().then("animation.villager.warrior_piglin.melee_attack2", Animation.LoopType.PLAY_ONCE))
        );
    }


    private <T extends GeoAnimatable> PlayState mainPredicate(AnimationState<T> state) {
        WarriorPiglinEntity self = (WarriorPiglinEntity) state.getAnimatable();
        Vec3d v = self.getVelocity();
        double speed = v.horizontalLengthSquared();

        var ctrl = state.getController();
        if (speed > 0.003) {
            ctrl.setAnimation(RawAnimation.begin().thenLoop("animation.villager.warrior_piglin.run"));
        } else if (speed > 1e-6) {
            ctrl.setAnimation(RawAnimation.begin().thenLoop("animation.villager.warrior_piglin.walk"));
        } else {
            ctrl.setAnimation(RawAnimation.begin().thenLoop("animation.villager.warrior_piglin.idle"));
        }
